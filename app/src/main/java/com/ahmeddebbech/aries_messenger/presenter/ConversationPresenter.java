...
public class ConversationPresenter extends Presenter implements ContractConversation.Presenter {
    ...
    public void sendMessage(String msg, String receiver) {
        if (!msg.equals("")) {
            if (!InputChecker.isLonger(msg, 1000)) {
                Message m = MessengerManager.getInstance().buildMessage(msg, receiver);

                DbConnector.connectToSendMessage(UserManager.getInstance().getAccessToken(), receiver, m, MessengerManager.getInstance().getCurrentConv(), this);

                activity.pushMessageToScreen(m);
                activity.clearField();
            } else {
    ...
    public void returnData(DatabaseOutput obj) {
        if(obj.getDatabaseOutputkey() == DatabaseOutputKeys.ANY_ERROR){
            AriesError u = (AriesError) obj.getObj();
            activity.showError(u);
        }
